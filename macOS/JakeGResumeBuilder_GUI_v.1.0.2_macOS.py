...
SESSION_RECOVERY_FILE_NAME = "unsaved_session.json"
...
def _session_data_dir() -> str:
    ...
    path = os.path.join(base, APP_NAME, "Session Data")
    try:
        os.makedirs(path, exist_ok=True)
    except Exception:
        pass
...
def _session_recovery_file_path() -> str:
    return os.path.join(_session_data_dir(), SESSION_RECOVERY_FILE_NAME)
...
def section_title_segments(section: Any) -> List[Dict[str, Any]]:
    if not isinstance(section, dict):
        return []
    tr = section.get("title_rich")
    if isinstance(tr, list):
        try:
            out = [dict(x) for x in tr if isinstance(x, dict)]
            for seg in out:
                seg["b"] = True
            return out
        except Exception:
            return []
    t = str(section.get("title", "") or "")
    if not t.strip():
        return []
    return [{"text": t, "b": True}]
...
def generate_latex(data: dict) -> str:
    ...
    for section in data.get("sections", []):
        title = rich_segments_to_latex(section_title_segments(section))
...
class RichTextEditor(ttk.Frame):
    def __init__(self, master, *, default_family: Optional[str] = None, default_size: Optional[int] = None):
        ...
        self.btn_strike = ttk.Button(toolbar, text="S", width=3, command=self.toggle_strike)

        self.btn_bold.pack(side=tk.LEFT, padx=(0, 4))
        self.btn_italic.pack(side=tk.LEFT, padx=(0, 4))
        self.btn_underline.pack(side=tk.LEFT, padx=(0, 4))
        self.btn_strike.pack(side=tk.LEFT, padx=(0, 8))

        ToolTip(self.btn_bold, "Bold")
        ToolTip(self.btn_italic, "Italic")
        ToolTip(self.btn_underline, "Underline")
        ToolTip(self.btn_strike, "Strikethrough")
        ...
        ToolTip(self.font_family_combo, "Font family")
        ...
        ToolTip(self.font_size_spin, "Font size")

        self.btn_text_color = ttk.Button(toolbar, text="Text Color", command=self.choose_fg)
        self.btn_text_color.pack(side=tk.LEFT, padx=(0, 6))
        ToolTip(self.btn_text_color, "Text color")

        self.btn_highlight = ttk.Button(toolbar, text="Highlight", command=self.choose_bg)
        self.btn_highlight.pack(side=tk.LEFT, padx=(0, 6))
        ToolTip(self.btn_highlight, "Highlight color")
    ...
    def _font_key_tag(self, st: StyleState) -> str:
        ...
        k = 1 if st.strike else 0
        return f"f:{st.family}|s:{st.size}|b:{b}|i:{i}|u:{u}|k:{k}"

    def _get_or_create_font(
        self, family: str, size: int, bold: bool, italic: bool, underline: bool, strike: bool
    ) -> tkfont.Font:
    ...
    def _style_at(self, index: str) -> StyleState:
        ...
        for t in tags:
            if t.startswith("f:"):
                try:
                    m = re.fullmatch(r"f:(.*)\|s:(\d+)\|b:(\d)\|i:(\d)\|u:(\d)\|k:(\d)", t)
                    if m:
                        family = m.group(1)
                        size = int(m.group(2))
                        bold = m.group(3) == "1"
    ...
    def _apply_style(
        self, *, family=None, size=None, bold=None, italic=None, underline=None, strike=None, fg=None, bg=None
    ) -> None:
        sel = self._selection_range()
        if not sel:
            return
        ...
        want_font_change = any(v is not None for v in (family, size, bold, italic, underline, strike))
        ...
        for rstart, rend, base in runs:
            if want_font_change:
                st = StyleState(
                    ...
                    strike=strike if strike is not None else base.strike,
                    fg=base.fg,
                    bg=base.bg,
                )
                self._remove_tag_prefix_in_range("f:", rstart, rend)
                font_tag = self._font_key_tag(st)
                fnt = self._get_or_create_font(st.family, st.size, st.bold, st.italic, st.underline, st.strike)
    ...
    def toggle_strike(self):
        idx = self._index_for_style_lookup()
        st = self._style_at(idx)
        self._apply_style(strike=not st.strike)
    ...
    def set_segments(self, segments):
        ...
        for seg in segments:
            ...
            st = StyleState(
                ...
                strike=bool(seg.get("s")),
                fg=seg.get("fg"),
                bg=seg.get("bg"),
            )

            font_tag = self._font_key_tag(st)
            f = self._get_or_create_font(st.family, st.size, st.bold, st.italic, st.underline, st.strike)
    ...
    def get_segments(self):
        ...
        def _style_from_font_tag(font_tag: Optional[str]):
            if not font_tag:
                return (self.default_family, self.default_size, False, False, False, False)
            m = re.fullmatch(r"f:(.*)\|s:(\d+)\|b:(\d)\|i:(\d)\|u:(\d)\|k:(\d)", font_tag)
            if not m:
                return (self.default_family, self.default_size, False, False, False, False)
            family = m.group(1)
            size = int(m.group(2))
        ...
        def _set_style_from_active():
            family, size, b, i, u, k = _style_from_font_tag(active_font_tag)
            cur["family"] = family
            cur["size"] = size
            cur["b"] = b
            cur["i"] = i
        ...
        for pos in range(0, len(text)):
            ...
            new_family, new_size, new_b, new_i, new_u, new_k = _style_from_font_tag(active_font_tag)
            new_style = {
                "family": new_family,
                "size": new_size,
                "b": new_b,
...
class BulletEditorDialog(tk.Toplevel):
    def __init__(
        ...
        *,
        ...
        ttk.Button(action, text="Cancel", command=self.on_cancel).pack(side=tk.RIGHT, padx=(6, 0))
        ttk.Button(action, text="OK", command=self.on_ok).pack(side=tk.RIGHT)
    ...
    def _notify_change(self) -> None:
        if self._on_change is None:
            return
        try:
            self._on_change(deep_copy(self.bullets))
        except Exception:
            return

    def _cancel_autosave_job(self) -> None:
        if self._autosave_job is None:
            return
        try:
    ...
    def _schedule_autosave_active(self, delay_ms: int = 450) -> None:
        self._cancel_autosave_job()
        try:
            self._autosave_job = self.after(delay_ms, self._autosave_active)
        except Exception:
            self._autosave_job = None

    def _autosave_active(self) -> None:
        self._autosave_job = None
        if not self.dirty:
            return
        try:
    ...
    def on_editor_modified(self, _event):
        ...
        self.dirty = True
        self._update_save_button()
        self._schedule_autosave_active()
...
class PreviewDialog(tk.Toplevel):
    ...
    def _insert_segments(self, segments: List[Dict[str, Any]], *, base_family: str, base_size: int):
        for idx, seg in enumerate(segments or []):
            ...
            f = self._get_font(family, size, bold, italic, underline, strike)
    ...
    def _build_rendered_preview(self):
        ...
        name_font = self._get_font(base_family, base_size + 10, True, False, False, False)
        section_font = self._get_font(base_family, base_size + 2, True, False, False, False)
        subhead_font = self._get_font(base_family, base_size, True, False, False, False)
        italic_font = self._get_font(base_family, base_size, False, True, False, False)
        ...
        self.rendered_text.tag_configure(
            "contact",
            font=self._get_font(base_family, base_size, False, False, False, False),
            justify="center",
        )
        ...
        for section in self.data.get("sections", []):
            title_segments = section_title_segments(section)
            kind = section.get("kind")
            entries = section.get("entries", [])

            if title_segments:
                self._insert_segments(title_segments, base_family=base_family, base_size=base_size + 2)
                self.rendered_text.insert("end", "\n")
            else:
                self.rendered_text.insert("end", "\n", ("section",))
...
class EntryEditorDialog(tk.Toplevel):
    def __init__(
        ...
        *,
        ...
        def add_field(label: str, key: str):
            ...
            try:
                var.trace_add("write", lambda *_: self._schedule_notify_change())
            except Exception:
                pass
        ...
        if section_kind in ("experience", "projects"):
            ...
            try:
                ...
                pass
            self.editor.text.bind("<<Modified>>", self._on_rich_modified)
    ...
    def _cancel_notify_job(self) -> None:
    ...
    def _schedule_notify_change(self, delay_ms: int = 300) -> None:
        if self._on_change is None:
            return
        self._cancel_notify_job()
        try:
            self._notify_job = self.after(delay_ms, self._notify_change)
        except Exception:
            self._notify_job = None
    ...
    def _notify_change(self) -> None:
        ...
        try:
            ...
            if self.section_kind in ("experience", "projects"):
                out["bullets"] = deep_copy(getattr(self, "bullets", []))
    ...
    def _on_rich_modified(self, _event) -> None:
        if self._on_change is None:
            return
        try:
            if not self.editor or not self.editor.text.edit_modified():
                return
            self.editor.text.edit_modified(False)
        except Exception:
            pass
        self._schedule_notify_change()

    def edit_bullets(self):
        before = deep_copy(self.bullets)

        def _live(bullets: list) -> None:
            try:
                self.bullets = deep_copy(bullets)
            except Exception:
            ...
            if self.bullets_button is not None:
                try:
                    ...
                    pass
            self._schedule_notify_change(0)

        dlg = BulletEditorDialog(self, "Edit Bullets", self.bullets, on_change=_live)
...
class ResumeApp(ttk.Frame):
    def __init__(self, master):
        ...
        self._session_last_saved_hash: Optional[str] = None
        self._session_draft_entry_sidx: Optional[int] = None
        self._session_draft_entry_eidx: Optional[int] = None
        self._session_draft_entry_is_new: bool = False
        self._session_draft_entry_value: Optional[dict] = None
        self._custom_label_prompt_active: bool = False
        self._custom_label_prompt_win: Optional[tk.Toplevel] = None
        ...
        self._init_session_recovery()
        self.refresh_sections()

        try:
            windowing = self.master.tk.call("tk", "windowingsystem")
        except Exception:
            windowing = ""
        if windowing == "aqua" or sys.platform == "darwin":
            try:
                self.master.after(0, self._apply_macos_startup_layout)
                self.master.after(120, self._apply_macos_startup_layout)
            except Exception:
                pass

    def _apply_macos_startup_layout(self) -> None:
        try:
            windowing = self.master.tk.call("tk", "windowingsystem")
        except Exception:
            windowing = ""
        if not (windowing == "aqua" or sys.platform == "darwin"):
            return

        if not hasattr(self, "outer_panes"):
            return
        ...
        try:
            w = int(self.master.winfo_width() or 0)
            h = int(self.master.winfo_height() or 0)
        except Exception:
            w = 0
            h = 0
        ...
        try:
            if w < 1100 or h < 720:
                ...
                self.master.update_idletasks()
                w = int(self.master.winfo_width() or w)
        except Exception:
            pass

        desired_left = 320
        if w > 0:
            desired_left = max(300, min(420, int(round(w * 0.30))))

        try:
            self.outer_panes.pane(self.outer_left_pane, minsize=280)
        except Exception:
            pass

        try:
            self.outer_panes.sashpos(0, desired_left)
        except Exception:
            try:
                self.outer_panes.tk.call(str(self.outer_panes), "sashpos", 0, desired_left)
            except Exception:
                pass

        try:
            self.sections_tree.column("kind", width=110, stretch=False)
        except Exception:
            pass

    def _compute_session_hash(self, state: Optional[dict] = None) -> str:
        st = self._state_with_session_draft() if state is None else state
        try:
            payload = json.dumps(st, sort_keys=True, ensure_ascii=False, separators=(",", ":"))
        except Exception:
            payload = json.dumps(deep_copy(st), sort_keys=True, ensure_ascii=False)
        return hashlib.sha256(payload.encode("utf-8", errors="ignore")).hexdigest()

    def _session_recovery_payload(self) -> dict:
        st = self._state_with_session_draft()
        payload = {
            "app_name": APP_NAME,
        ...
        try:
            payload["state"]["spellcheck_ignore_all"] = self.spellcheck_manager.get_ignore_all()
        except Exception:
            pass
        return payload

    def _state_with_session_draft(self) -> dict:
        st = deep_copy(self.data)
        if self._session_draft_entry_value is None:
            return st
        if self._session_draft_entry_sidx is None or self._session_draft_entry_eidx is None:
            return st
        try:
            sidx = int(self._session_draft_entry_sidx)
            eidx = int(self._session_draft_entry_eidx)
        except Exception:
            return st
        try:
            secs = st.get("sections", [])
            if not (0 <= sidx < len(secs)):
                return st
            sec = secs[sidx]
            entries = list(sec.get("entries", []) or [])
            if self._session_draft_entry_is_new:
                ins = max(0, min(eidx, len(entries)))
                entries.insert(ins, deep_copy(self._session_draft_entry_value))
            else:
                if not (0 <= eidx < len(entries)):
                    return st
                entries[eidx] = deep_copy(self._session_draft_entry_value)
            sec["entries"] = entries
        except Exception:
            return st
        return st

    def _entry_has_any_content(self, entry: Optional[dict]) -> bool:
    ...
    def _set_session_draft_entry(self, *, sidx: int, eidx: int, is_new: bool, entry: dict) -> None:
        if is_new and not self._entry_has_any_content(entry):
            self._clear_session_draft_entry()
            return
        self._session_draft_entry_sidx = int(sidx)
        self._session_draft_entry_eidx = int(eidx)
        self._session_draft_entry_is_new = bool(is_new)
        self._session_draft_entry_value = deep_copy(entry)
        self._schedule_session_autosave()

    def _clear_session_draft_entry(self) -> None:
        had = self._session_draft_entry_value is not None
        self._session_draft_entry_sidx = None
        self._session_draft_entry_eidx = None
        self._session_draft_entry_is_new = False
        self._session_draft_entry_value = None
        if had:
            self._schedule_session_autosave()

    def _delete_session_recovery_file(self) -> None:
        path = _session_recovery_file_path()
        try:
    ...
    def _write_session_recovery_file(self) -> None:
        path = _session_recovery_file_path()
        tmp_path = path + ".tmp"
        try:
        ...
        try:
            with open(tmp_path, "w", encoding="utf-8") as f:
                json.dump(self._session_recovery_payload(), f, indent=2, ensure_ascii=False)
    ...
    def _mark_session_saved(self) -> None:
        try:
            if self._session_autosave_job is not None:
                try:
                    self.master.after_cancel(self._session_autosave_job)
                except Exception:
                    pass
        finally:
            self._session_autosave_job = None
        ...
        try:
            self._session_last_saved_hash = self._compute_session_hash(self.data)
        except Exception:
            self._session_last_saved_hash = None

        if self._session_draft_entry_value is None:
            self._delete_session_recovery_file()

    def _schedule_session_autosave(self, delay_ms: int = 1250) -> None:
        ...
        try:
            self._session_autosave_job = self.master.after(delay_ms, self._autosave_session)
        except Exception:
            self._session_autosave_job = None
    ...
    def _autosave_session(self) -> None:
        ...
        try:
            cur_hash = self._compute_session_hash()
        except Exception:
            return

        if self._session_last_saved_hash is not None and cur_hash == self._session_last_saved_hash:
            self._delete_session_recovery_file()
            return

        self._write_session_recovery_file()

    def _init_session_recovery(self) -> None:
        try:
            self._session_last_saved_hash = self._compute_session_hash()
        except Exception:
            self._session_last_saved_hash = None
        self._maybe_prompt_recover_session()

    def _ask_custom_label(self, title: str, prompt: str) -> Optional[str]:
        if self._custom_label_prompt_active:
            try:
                if self._custom_label_prompt_win is not None and self._custom_label_prompt_win.winfo_exists():
                    self._custom_label_prompt_win.lift()
                    self._custom_label_prompt_win.focus_force()
            except Exception:
                pass
            return None

        self._custom_label_prompt_active = True
        result: Dict[str, Optional[str]] = {"value": None}

        dlg = tk.Toplevel(self.master)
        self._custom_label_prompt_win = dlg
        dlg.title(title)
        try:
        ...
        try:
            ...
            self._custom_label_prompt_active = False
            self._custom_label_prompt_win = None
    ...
    def _prompt_recover_session(self) -> bool:
        result: Dict[str, Optional[bool]] = {"value": None}

        dlg = tk.Toplevel(self.master)
        dlg.title("Recover Session")
        try:
            dlg.transient(self.master)
        except Exception:
            pass
        ...
        ttk.Label(root, text="An unsaved session was found from a previous run.\n\nRecover it?").pack(
            anchor="w"
        )
        ...
        btn_no = ttk.Button(btns, text="No", command=_no)
        btn_no.pack(side=tk.RIGHT, padx=(6, 0))
        ToolTip(btn_no, "This deletes all recovered data when selected")
        ttk.Button(btns, text="Yes", command=_yes).pack(side=tk.RIGHT)
    ...
    def _prompt_section_title_rich(self, dialog_title: str, *, initial_segments: Optional[list] = None) -> Optional[list]:
        result: Dict[str, Optional[list]] = {"value": None}
        ...
        ttk.Label(root, text="Section title:").pack(anchor="w")
        ed = RichTextEditor(root)
        ed.pack(fill=tk.BOTH, expand=True, pady=(6, 10))

        try:
            ed.set_segments(initial_segments or [])
        ...
        try:
            ...
            pass
        try:
            ed.text.bind("<Return>", lambda _e: (_ok(), "break")[1])
            ed.text.bind("<KP_Enter>", lambda _e: (_ok(), "break")[1])
        except Exception:
            pass
    ...
    def _prompt_backup_or_clear_recovered_data(self) -> Optional[str]:
        result: Dict[str, Optional[str]] = {"value": None}

        dlg = tk.Toplevel(self.master)
        dlg.title("Back up Data to Project File")
        try:
            dlg.transient(self.master)
        except Exception:
            pass
        ...
        ttk.Label(
            root,
            text=(
                "Are you sure you don't want to recover information from a prior session?\n\n"
                "Choose 'Back up Data to Project File' to save the recovered data as a project file.\n"
                "Choose 'No' to erase all recovered data."
            ),
        ).pack(anchor="w")
        ...
        ttk.Button(btns, text="No", command=_clear).pack(side=tk.RIGHT, padx=(6, 0))
        ttk.Button(btns, text="Back up Data to Project File", command=_backup).pack(side=tk.RIGHT)
        ...
        try:
            ...
            pass
        try:
            dlg.bind("<Return>", lambda _e: _backup())
            dlg.bind("<Escape>", lambda _e: _cancel())
        except Exception:
            pass
    ...
    def _maybe_prompt_recover_session(self) -> None:
        path = _session_recovery_file_path()
        try:
            if not os.path.exists(path):
                return
        ...
        recovered_project_path: Optional[str] = None

        if isinstance(raw, dict) and isinstance(raw.get("state"), dict):
            state = raw.get("state")
            try:
                recovered_project_path = raw.get("current_project_path")
            except Exception:
        ...
        ok = self._prompt_recover_session()
        if not ok:
            choice = self._prompt_backup_or_clear_recovered_data()
            if choice == "clear":
                self._delete_session_recovery_file()
                return
            if choice != "backup":
            ...
            save_path = filedialog.asksaveasfilename(
                parent=self,
                title="Save Recovered Session as Project",
                defaultextension=".json",
                initialdir=_workspace_path(),
                initialfile="Recovered Session.json",
            ...
            self._delete_session_recovery_file()
            messagebox.showinfo("Saved", f"Saved recovered session to {save_path}")
            return
        ...
        if recovered_project_path:
            try:
                self.current_project_path = str(recovered_project_path)
            except Exception:
    ...
    def _build_ui(self):
        ...
        ToolTip(file_btn, "File actions (Save, Load, Export)")
        ...
        ToolTip(edit_btn, "Edit actions (Undo, Redo, Delete all)")
        ...
        ToolTip(help_btn, "Help and documentation")
        ...
        self.btn_delete_all = ttk.Button(topbar_row2, text="Delete all", command=self.delete_all)
        self.btn_delete_all.pack(side=tk.LEFT, padx=(0, 6))
        ToolTip(self.btn_delete_all, "Clear all entered data")

        self.btn_preview = ttk.Button(topbar_row2, text="Preview", command=self.preview)
        self.btn_preview.pack(side=tk.LEFT, padx=(0, 6))
        ToolTip(self.btn_preview, "Open preview")

        ttk.Separator(topbar_row2, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=8)

        self.btn_demo_mode = ttk.Button(topbar_row2, text="Demo mode", command=self.demo_mode)
        self.btn_demo_mode.pack(side=tk.LEFT, padx=(0, 6))
        ToolTip(self.btn_demo_mode, "Load demo data")
        ...
        self.outer_panes = outer
        self.outer_left_pane = left
        self.outer_right_pane = right
        ...
        def add_header_link_field(row_parent, kind_key: str, url_key: str, kind_values: Tuple[str, ...], default_kind: str):
            ...
            def _on_kind_selected(_event: Any = None) -> None:
                ...
                if selected.lower() == "custom":
                    ...
                    custom = self._ask_custom_label(prompt_title, prompt_label)
                    if not custom:
                        kind_var.set(prev)
                        return
                    else:
        ...
        self.btn_section_add = ttk.Button(sec_btns, text="Add", command=self.add_section)
        self.btn_section_add.pack(side=tk.LEFT, padx=(0, 6))
        ToolTip(self.btn_section_add, "Add a section")
        self.btn_section_edit = ttk.Button(sec_btns, text="Edit", command=self.rename_section)
        self.btn_section_edit.pack(side=tk.LEFT, padx=(0, 6))
        ToolTip(self.btn_section_edit, "Edit section title")
        self.btn_section_delete = ttk.Button(sec_btns, text="Delete", command=self.delete_section)
        self.btn_section_delete.pack(side=tk.LEFT)
        ToolTip(self.btn_section_delete, "Delete selected section")
        ...
        ToolTip(self.btn_entry_add, "Add an entry")
    ...
    def export_as_docx(self):
        ...
        def _add_rich_runs(p, segments: Any) -> None:
            ...
            for seg in segments:
                ...
                try:
                    run.font.strike = bool(seg.get("s"))
                except Exception:
                    pass
                if seg.get("font"):
                    try:
        ...
        def _add_section_header(section_obj: Any) -> None:
            title_segs = section_title_segments(section_obj)
            if not title_segs:
                return
            p = add_rich_paragraph(doc, title_segs)
            try:
                for r in p.runs:
                    r.bold = True if r.bold is None else r.bold
                    r.font.name = "Times New Roman"
                    r.font.size = Pt(11)
            except Exception:
                pass
    ...
    def export_as_pdf(self):
        ...
        def _add_section_header(story: List[Any], section_obj: Any) -> None:
            segs = section_title_segments(section_obj)
            if not segs:
                return
            p = segments_to_paragraph(segs, section_style)
            if p is not None:
                story.append(p)
            story.append(
                HRFlowable(
        ...
        for section in self.data.get("sections", []) or []:
            ...
            _add_section_header(story, section)
    ...
    def add_section(self):
        title_rich = self._prompt_section_title_rich("Add Section")
        if not title_rich:
            return
        title_plain = "".join([str(s.get("text", "")) for s in title_rich if isinstance(s, dict)])
        title_plain = title_plain.replace("\r", " ").replace("\n", " ").strip()
        if not title_plain:
            return
        ...
        self._checkpoint_before_change()
        sec = {
            "id": f"sec_{len(self.data.get('sections', [])) + 1}",
            "title": title_plain,
            "title_rich": deep_copy(title_rich),
            "kind": kind,
            "entries": [],
        }
    ...
    def rename_section(self):
        ...
        initial = sec.get("title_rich") if isinstance(sec.get("title_rich"), list) else [{"text": str(sec.get("title", "")), "b": True}]
        new_title_rich = self._prompt_section_title_rich("Edit Section", initial_segments=initial)
        if not new_title_rich:
            return
        new_title_plain = "".join([str(s.get("text", "")) for s in new_title_rich if isinstance(s, dict)])
        new_title_plain = new_title_plain.replace("\r", " ").replace("\n", " ").strip()
        if not new_title_plain:
            return
        self._checkpoint_before_change()
        sec["title"] = new_title_plain
        sec["title_rich"] = deep_copy(new_title_rich)
        self.refresh_sections()
    ...
    def add_entry(self):
        ...
        draft_eidx = len(entries)

        def _draft(entry: dict) -> None:
            self._set_session_draft_entry(sidx=sidx, eidx=draft_eidx, is_new=True, entry=entry)

        dlg = EntryEditorDialog(self, kind, new_entry, on_change=_draft)
        self.wait_window(dlg)
    ...
    def edit_entry(self):
        ...
        def _draft(entry: dict) -> None:
            self._set_session_draft_entry(sidx=sidx, eidx=eidx, is_new=False, entry=entry)

        dlg = EntryEditorDialog(self, kind, entries[eidx], on_change=_draft)
        self.wait_window(dlg)
